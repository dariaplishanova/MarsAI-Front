...
export default function JuryDashboard() {
  ...
  const filteredFilms = safeFilmsList.filter(film => film.title?.toLowerCase().includes(searchQuery.toLowerCase()));

  const activeFilm = safeFilmsList.find(film => film.id.toString() === activeFilmId);
  ...
  if (error && safeFilmsList.length === 0) {
    return <div className="text-destructive flex h-screen items-center justify-center">{error}</div>;
  }

  return (
    <div className="bg-background flex h-screen overflow-hidden">
      <aside className="border-border bg-card hidden h-screen w-80 flex-col border-r p-6 md:flex">
        <div className="mb-6 shrink-0">
          <h2 className="text-foreground text-xl font-bold">{t('jury.dashboard.title')}</h2>
      ...
      <main className="flex flex-1 flex-col overflow-hidden">
        <header className="border-border bg-card flex items-center justify-between border-b p-4 md:hidden">
          <h1 className="text-foreground text-lg font-bold">{t('jury.dashboard.mobile_title')}</h1>
          <MobileSidebar
            films={filteredFilms}
            activeFilmId={activeFilmId}
            onSelectFilm={setActiveFilmId}
